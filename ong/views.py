...
def get_geoactions(request):
    actions = action.objects.all()
    #data = serializers.serialize('geojson', actions, srid='4326', fields = ( 'titre' , 'date', 'description' , 'organisme' , 'categories' , 'localisation', 'illustration', 'responsable', 'avancement', 'geom',))
    data = serializers.serialize('geojson', actions, srid='4326',)
